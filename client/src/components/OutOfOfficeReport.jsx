// src/components/OutOfOfficeReport.jsx
...
/* ---------- NEW: time-range helper ---------- */
const isWithinTimeRange = (dateStr, fromTimeStr, toTimeStr) => {
  if (!dateStr || !fromTimeStr || !toTimeStr) return true; // no limit
...
const OutOfOfficeReport = () => {
  ...
  /* ---------- NEW: Out-of-office time filter state ---------- */
  const [timeRange, setTimeRange] = useState({
    from: "18:00", // default 6:00 PM
    to: "06:00", // default 6:00 AM
  });
  ...
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      ...
      /* ---------- NEW: Out-of-office filter by Start Time (begin) ---------- */
      if (
        timeRange.from &&
        timeRange.to &&
        !isWithinTimeRange(item.begin, timeRange.from, timeRange.to)
      ) {
        return false;
      }
  ...
  }, [data, filters, dateFilters, timeRange]);
        ...
        <div className="card">
          <div className="d-flex mb-2 px-4 pt-3 justify-content-between">
            <div className="table-title">
              <h5 className="fw-bold py-2">Office hour Report</h5>
            </div>
          ...
          <div className="modal-dialog modal-lg">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Office Hours Report</h5>
                <button
                  type="button"
                  className="btn-close"
              ...
              <div className="modal-body">
                ...
                {/* ---------- NEW: Time Range (Start Time) ---------- */}
                <div className="mb-4">
                  <h6 className="mb-2">Time Range (Start Time)</h6>
                  <div className="d-flex gap-2 flex-wrap align-items-center">
                    ...
                    <input
                      ...
                      style={{ width: 160 }}
                    />
                    <label className="ms-3 me-1">To:</label>
                    <input
                      type="time"
                      className="form-control"
                      value={timeRange.to}
                    ...
                    <span className="text-muted ms-2">
                      (Overnight windows like 18:00 → 06:00 are supported)
                    </span>
                  </div>
                </div>
...
export default OutOfOfficeReport;
